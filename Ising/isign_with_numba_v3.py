...
@jit(nopython=True)
def passo(color, mag, E, viz):
    for i in range(L2/2):
             sitio =  c[color][i]# vou escolher um sitio aleatorio
...
def dinamica(s):
        ...
        c = checkboard()  # inicia checkerboard
        E, mag = init_medidas(s, viz) # iniciando energia e mag
        
        """ DAR UM JEITO DE PASSAR APENAS UMA COR PRA PASSO()"""
