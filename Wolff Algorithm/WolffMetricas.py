...
for temp in TEMp:
    ...
    for t in range(TMAX):
        ...
        mag = cluster_din(sitio, mag)  
        
        #adicionando o valor da magnetização
        metricas.append(mag)
        ...
        lista_cluster_size.append(abs(cluster_size))
    #print(time.process_time()-c, "Segundos")
    
    #tamanho medio do cluster
...
# plt.title(r"Série Temporal da Magnetização para $T_c$", fontsize=30)

# plt.plot(metricas, label='Wolff',  lw=2)
# plt.plot(medidas7, label='Metropolis', lw=2)
# plt.xlabel('MCsteps', fontsize=20)
...
# """GRAFICO DA PROB DE ACEITAÇÃO"""

# x  = np.linspace(0.05, 10,500)

# plt.plot(x, (1-np.exp(-2/x)), label=r'$f(T)=1-e^{-2/T}$', lw=3, c='navy')
# #plt.scatter(x, (1-np.exp(-2/x)), c='navy', s=0.5)
# plt.legend(fontsize=20,loc='best')
# plt.xlabel("$T$", fontsize=20)
# plt.title("$P_{add}$ em função da Temperatura", fontsize=30)
